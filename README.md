...
## วิดีโอ motion comic draft

ไฟล์ `.github/workflows/video-draft.yml` เป็น workflow แยกจากการโพสต์ภาพเดิม โดยจะทำงานตามเวลาไทย 07:00, 10:00, 12:00, 15:00, 18:00 และ 20:00 จากข่าวนักเตะที่ผ่านการตรวจสอบและมีรูปจริง ระบบจะสรุปข่าวและสร้าง prompt ภาพการ์ตูนล้อเลียน 4 ฉากด้วยภาษาไทย/อังกฤษตามชนิดข้อมูล จากนั้นสร้างภาพการ์ตูน AI แยกทั้ง 4 ฉาก นำภาพมาทำ motion comic ความยาว 45 วินาที อัตราส่วนแนวตั้ง 1080×1920 และอัปโหลดวิดีโอ ภาพแต่ละฉาก และ JSON storyboard เป็น GitHub Actions artifact สำหรับตรวจสอบเท่านั้น Workflow นี้ไม่มี `FB_PAGE_ID` หรือ `FB_PAGE_TOKEN` และจะไม่โพสต์วิดีโอไป Facebook

หลัง workflow ทำงานเสร็จ ให้เปิด run ของ `Football video draft` แล้วดาวน์โหลด artifact ชื่อ `football-video-draft-<run-id>` ภายในมีไฟล์ `.mp4`, ภาพ `scene_01.png` ถึง `scene_04.png` และ `.json` ซึ่งเก็บข้อมูลข่าว prompt ของแต่ละฉาก แหล่งรูป มุก และสถานะ `draft_pending_review` ก่อนอนุมัติควรตรวจชื่อผู้เล่น ตัวเลข ความหมายของข่าว ความเหมาะสมของมุก คุณภาพภาพทั้ง 4 ฉาก ความยาววิดีโอ และสิทธิ์การใช้งานสื่อ

หากต้องการสร้าง draft ด้วยตนเอง ให้ใช้คำสั่ง `python video_draft.py` หลังตั้งค่า `OPENAI_API_KEY` และ `FONT_PATH` แล้ว ไฟล์จะถูกเก็บไว้ใน `video_drafts/` และจะไม่ถูก commit เข้า repository
