...
ไฟล์ `.github/workflows/video-draft.yml` เป็น workflow แยกจากการโพสต์ภาพเดิม โดยจะทำงานตามเวลาไทย 07:00, 10:00, 12:00, 15:00, 18:00 และ 20:00 จากข่าวนักเตะที่มีรูปจริง ระบบจะสร้างวิดีโอแนว motion comic ความยาว 45 วินาที อัตราส่วนแนวตั้ง 1080×1920 พร้อมมุกตลกภาษาไทย 3 ฉาก แล้วอัปโหลดเป็น GitHub Actions artifact สำหรับตรวจสอบเท่านั้น Workflow นี้ไม่มี `FB_PAGE_ID` หรือ `FB_PAGE_TOKEN` และจะไม่โพสต์วิดีโอไป Facebook

หลัง workflow ทำงานเสร็จ ให้เปิด run ของ `Football video draft` แล้วดาวน์โหลด artifact ชื่อ `football-video-draft-<run-id>` ภายในมีไฟล์ `.mp4` และ `.json` ซึ่งเก็บข้อมูลข่าว แหล่งรูป มุก และสถานะ `draft_pending_review` ก่อนอนุมัติควรตรวจชื่อผู้เล่น ตัวเลข ความหมายของข่าว ความเหมาะสมของมุก คุณภาพภาพ ความยาววิดีโอ และสิทธิ์การใช้งานรูปภาพ
